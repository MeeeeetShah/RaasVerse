  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-hidden">
      ...
      <div className="relative w-full max-w-4xl rounded-3xl bg-[#0d0e14] border border-amber-500/30 shadow-[0_25px_60px_rgba(0,0,0,0.9)] overflow-hidden z-10 flex flex-col max-h-[90vh]">
        ...
        {/* Venue Amenities & Features Bar */}
        {((event.venueFeatures && event.venueFeatures.length > 0) || (event.features && event.features.length > 0)) && (
          <div className="px-5 py-2.5 bg-black/70 border-t border-white/10 flex flex-wrap items-center gap-2">
            <span className="text-[11px] font-bold text-amber-400 uppercase tracking-wider shrink-0 flex items-center gap-1">
              <Sparkles className="w-3 h-3" /> Ground Facilities:
            </span>
            {(event.venueFeatures || event.features).map((feat, idx) => (
              <span key={idx} className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-white/5 border border-white/10 text-gray-200">
                ✓ {feat}
              </span>
            ))}
          </div>
